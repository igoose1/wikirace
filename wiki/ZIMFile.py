...
Zimply tries to log in our directory; lib also implements own settings
...
class ZIMFile:
    ...
    def __init__(self, filename, index_filename, encoding='utf-8'):
        ...
        self._good_article_count = os.path.getsize(index_filename) // settings.ZIM_BLOCK_SIZE

    def __enter__(self):
        return self

    def random_article(self):
        offset = randrange(0, self._good_article_count) * settings.ZIM_BLOCK_SIZE
        self._article_indexes.seek(offset)
        index = struct.unpack(settings.UNPACK_FORMAT, self._article_indexes.read(settings.ZIM_BLOCK_SIZE))[0]
        return self[index]
