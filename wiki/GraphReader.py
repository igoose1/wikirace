...
def read_int_from_file(file, block_size: int) -> int:
    ...
    return struct.unpack(settings.UNPACK_FORMAT, byte)[0]
...
class GraphReader:
    ...
    def _get_offset_by_id(self, parent_id: int):
        self.offset_file.seek(settings.OFFSET_BLOCK_SIZE * parent_id)
        offset_begin = read_int_from_file(self.offset_file, settings.OFFSET_BLOCK_SIZE)
        offset_end = read_int_from_file(self.offset_file, settings.OFFSET_BLOCK_SIZE)
        return offset_begin, offset_end

    def edges_count(self, parent_id: int):
        offset_begin, offset_end = self._get_offset_by_id(parent_id)
        return (offset_end - offset_begin) // settings.EDGE_BLOCK_SIZE
    ...
    def edges(self, parent_id: int):
        ...
        for idx in range(self.edges_count(parent_id)):
            child_id = read_int_from_file(self.edges_file, settings.EDGE_BLOCK_SIZE)
            yield child_id
